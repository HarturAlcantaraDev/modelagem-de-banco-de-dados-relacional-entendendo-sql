...
ON DELETE NO ACTION 	#comando para gerear erro toda vez que tentar deletar alguma informação que não esteja tabela livro
ON UPDATE NO ACTION;	#comando para gerar erro toda vez que tentar alterar um livro que estiver na tabela estoque mas não estiver na tabela estoque
...
FROM TB_VENDA ORDER BY ID_VENDEDORES;
...
ON VENDAS.ID_LIVRO  = VENDAS.ID_LIVRO
WHERE VENDAS.QTD_VENDIDA IS NULL;

SELECT LIVROS.NOME_LIVRO, VENDAS.QTD_VENDIDA
FROM LIVROS RIGHT JOIN VENDAS
ON VENDAS.ID_LIVRO  = VENDAS.ID_LIVRO
WHERE VENDAS.QTD_VENDIDA IS NOT NULL;
